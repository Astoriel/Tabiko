  ...
  function addEventListeners() {
        ...
        if (itemTarget) {
            ...
            if (item) {
                ...
                contextMenu.innerHTML = `
                    ...
                    <div class="menu-item" data-action="replace_with_current">Replace with current</div>
    ...
    contextMenu.addEventListener('click', async (e) => {
        ...
        if (contextTarget.type === 'link') {
            ...
            } else if (action === 'replace_with_current') {
                const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
                if (tabs && tabs.length > 0) {
                    const activeTab = tabs[0];
                    item.url = activeTab.url || activeTab.pendingUrl || item.url;
                    item.title = activeTab.title || item.url;
                }
            }
        }
        // --- Workspace Actions ---
